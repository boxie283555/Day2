...
def login_and_execcommand_bak(name,ip_address,user,passwd,platform,login_mode):
     login_flags = bool()
     tn = telnetlib.Telnet()
     i = 0
     for i in range(3):
...
def login_and_execcommand_crs(name,ip_address,user,passwd,platform,login_mode):
     ...
     for i in range(3):
          ...
          resp = channel.recv(RECE_DATA).decode('utf-8')
     ...
     if login_flags == True and platform == "CRS":
          ...
          command_result_clock = channel.recv(RECE_DATA).decode('utf-8')
          flags = True
          while  flags:
          ...
          command_result_hostname = channel.recv(RECE_DATA).decode('utf-8')
          flags_host = True
          while  flags_host:
          ...
          for command_key in crs_command:
               ...
               time.sleep(WAITE_COMMAND_TIME)
               command_result = ""
               command_result = channel.recv(RECE_DATA).decode('utf-8')
               flags_command = True
               while  flags_command:
          ...
          channel.send(command_rackname + "\n")
          time.sleep(WAITE_COMMAND_TIME)
          command_result_rackname=""
          command_result_rackname = channel.recv(RECE_DATA).decode('utf-8')

          flags_rack = True
          while  flags_rack:
               if command_result_rackname.rstrip().endswith("#"):
                    flags_rack = False
               else:
                    time.sleep(WAITE_COMMAND_TIME)
                    command_result_rackname += channel.recv(RECE_DATA).decode('utf-8')

          rackfile.write(command_result_rackname)
          time.sleep(WAITE_FILE_TIME)
          rackfile.close()
          ...
          for command_key in a9k_command:
               ...
               command_txt_filename = name + "%" + exec_command + "%" + str(create_file_time[0]) + ".txt"
               command_file = open(command_txt_filename,'w')
               channel.send(a9k_command[command_key] + "\n")
               time.sleep(WAITE_COMMAND_TIME)
               command_result = channel.recv(RECE_DATA).decode('utf-8')
               flags_command = True
               while  flags_command:
