...
#current_dir = str(os.getcwd())+ "/9929/"
...
#os.environ["NET_TEXTFSM"] = "/Users/boxie2/Desktop/python/struct-telnet/4837/templates"

WAITE_FILE_TIME = 5
WAITE_COMMAND_TIME = 5 
...
def import_index_a9k_mapping(es_address,key_es):
     ...
     host = set_eshost[key_es]
     elastic = Elasticsearch(hosts= host)
     for each_index in a9k_jsondir:
          index_database = a9k_jsondir[each_index] + index_ext_time
          url = es_address  + index_database
          res = requests.get(url)
          if res.status_code != 200:
               mapping_file = a9k_jsondir[each_index] + "mapping.json"
               with open(mapping_file, 'r', encoding='utf-8') as f:
                    data=json.loads(f.read())
               ...
               if 'acknowledged' in response:
                    if response['acknowledged'] == True:
                         logger.info('import_index_a9k_mapping:{} INDEX MAPPING SUCCESS FOR INDEX'.format(response['index']))

               elif 'error' in response:
                    logger.info('import_index_a9k_mapping:ERROR {}'.format(response['error']['root_cause']))
                    logger.info('import_index_a9k_mapping:TYPE {}'.format(response['error']['type']))

               logger.info('import_index_a9K_mapping:response {}'.format(response))
...
def timeconvert(name,showtime):
     ...
     elif  "AEDT" in str(showtime):
          showtime = showtime.replace("AEDT", "CST")
     else:
          pass
     try:     
...
def login_and_execcommand(name,ip_address,user,passwd,platform,login_mode):
     ...
     if login_flags == True and platform == "CRS":  
     ...
     elif login_flags == True and platform == "A9K":  
          dict3={} 
          file_list = []
          create_file_time = get_nowtime()
          #add clock 
          ...
          #add Platform
          command_rackname = "show inventory rack"
          new_command_rackname = command_rackname.replace(" ","_")
          rackname_txt_filename = name + "%" + new_command_rackname + "%" + str(create_file_time[0]) + ".txt"
          rackfile = open(rackname_txt_filename,'w')
          tn.write(command_rackname.encode("ascii") + b'\n')
          time.sleep(WAITE_COMMAND_TIME)
          result = tn.read_very_eager().decode("ascii")
          flags_rack = True
          while  flags_rack:
               if result[-1] == "#":
                    ...
                    time.sleep(WAITE_COMMAND_TIME)
                    result += tn.read_very_eager().decode("ascii")
          rackfile.write(result)
          time.sleep(WAITE_FILE_TIME)
          rackfile.close()
          output_rackname = run(show_file=rackname_txt_filename, template_file=a9k_select_template[command_rackname])
          rackdict = {"Chassis_pid":""}
          for each_field in output_rackname:
               for key in each_field.keys():
                    if key == "Chassis_pid":
                         rackdict['Chassis_pid']=each_field[key]
...
def mv_a9k_files():
     ...
     try:
          ...
          logger.info('mv_files: {} directory is exists!!!'.format(clock_dir))
     try:
          rack_dir = "show_inventory_rack"
          os.mkdir(rack_dir)
     except:
          #print("directory is exists!!!")
          logger.info('mv_files: {} directory is exists!!!'.format(rack_dir))
     try:
          hostname_dir = "show_running_hostname"
          os.mkdir(hostname_dir)
     except:
     ...
     command_new_list.append(hostname_dir)
     command_new_list.append(power_dir)
     command_new_list.append(rack_dir)
...
if __name__ == '__main__':
    ...
    #print ("Total execution time",(datetime.datetime.now() - startTime))
     ...
     print("cpu",cpus)
     p = multiprocessing.Pool(cpus*65)
     for name,ip_address,user,passwd,platform,login_mode in zip(hostname,ip,username,password,platform_mode,login):
